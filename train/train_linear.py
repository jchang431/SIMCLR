...
def evaluate_confusion_matrix(model, loader, device, save_path):
    ...
    plt.title("SimCLR Linear Eval Normalized Confusion Matrix")
    plt.colorbar()
...
class LinearEvalTrainer(Trainer):
    """
    SimCLR linear evaluation trainer.
    - Loads pretrained SimCLR encoder
    - Freezes the full encoder
    - Trains only the linear classifier
    ...
    def train(self):
        ...
        best_ckpt_path = (
            f"{self.output_dir}/simclr_linear_{dataset_name}_{label_pct}pct_best.pth"
        )
        ...
        last_path = (
            f"{self.output_dir}/simclr_linear_{dataset_name}_{label_pct}pct_last.pth"
        )
        ...
        metrics = {
            ...
            "label_pct": label_pct,
            "experiment": "simclr_linear_eval_frozen_encoder",
            "encoder": "frozen",
        ...
        metrics_path = (
            f"{self.output_dir}/linear_metrics_{dataset_name}_{label_pct}pct.json"
        )
    ...
    def test(self):
        ...
        print(
            ...
            f"Loss {loss_meter.avg:.3f}\tAccuracy {acc:.2f}"
        )
        ...
        cm_path = (
            f"{self.output_dir}/cm_simclr_linear_{dataset_name}_{label_pct}pct.png"
        )
        ...
        results_path = (
            f"{self.output_dir}/linear_test_results_{dataset_name}_{label_pct}pct.json"
        )

        with open(results_path, "w") as f:
            json.dump(test_results, f, indent=2)

        print(f"Linear eval test results saved to {results_path}")