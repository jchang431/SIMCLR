...
def evaluate_confusion_matrix(model, loader, device, save_path):
    ...
    plt.title("SimCLR Full Fine-tune Normalized Confusion Matrix")
    plt.colorbar()
...
class FineTuneTrainer(Trainer):
    """
    SimCLR full fine-tuning trainer.

    - Loads pretrained SimCLR encoder checkpoint.
    - Does NOT freeze the encoder.
    - Trains full encoder + classifier.
    - Uses smaller LR for encoder and larger LR for classifier.
    ...
    def _init_optimizer(self):
        ...
        return torch.optim.SGD(
            [
                {"params": self.net.encoder.parameters(), "lr": encoder_lr},
    ...
    def train(self):
        ...
        best_ckpt_path = (
            f"{self.output_dir}/simclr_fullft_{dataset_name}_{label_pct}pct_best.pth"
        )
        ...
        last_path = (
            f"{self.output_dir}/simclr_fullft_{dataset_name}_{label_pct}pct_last.pth"
        )
        ...
        metrics = {
            ...
            "label_pct": label_pct,
            "experiment": "simclr_full_finetune",
            "encoder_lr": self.optimizer.param_groups[0]["lr"],
            "classifier_lr": self.optimizer.param_groups[1]["lr"],
            "scheduler": "CosineAnnealingLR",
            "trainable_encoder": "full_encoder",
        }

        metrics_path = (
            f"{self.output_dir}/fullft_metrics_{dataset_name}_{label_pct}pct.json"
        )
    ...
    def test(self):
        ...
        cm_path = (
            f"{self.output_dir}/cm_simclr_fullft_{dataset_name}_{label_pct}pct.png"
        )
        ...
        results_path = (
            f"{self.output_dir}/fullft_test_results_{dataset_name}_{label_pct}pct.json"
        )

        with open(results_path, "w") as f:
            json.dump(test_results, f, indent=2)

        print(f"Fine-tune test results saved to {results_path}")